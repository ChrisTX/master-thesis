...
\end{theorem}
\begin{proof}
This proof follows ideas presented in \cite{Hinze}.
...
\noalign{\noindent By the definition of the $\lDG \cdot \rDG$ norm, we have the term $\| f(T) \|_{L^2(\Omega)}$ in it, we can estimate \vspace{\jot}}
	&\overset{\text{\cref{eq:dg-DG-norm}}}{\leq}& \left\lDG \bar{y} - \bar{y}^h \right\rDG \| y^v(T) \|_{L^2(\Omega)} + \left( \left\lDG \bar{y}^h \right\rDG + \| y_\Omega \|_{L^2(\Omega)} \right) \left\lDG y^v_h - y_v \right\rDG \\
\noalign{\noindent Due to the continuity estimate on exact solutions given by \cref{thm:generalized-problem-solution} and the fact that $W(0, T) \hookrightarrow C([0, T], L^2(\Omega))$, we estimate this further as\vspace{\jot}}
