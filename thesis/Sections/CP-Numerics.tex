...
\noalign{\noindent By the definition of the $\lDG f \rDG$ norm, we have the term $\| f(T) \|_{L^2(\Omega)}$ in it, c.f.\ \cref{eq:dg-DG-norm}, we can estimate after employing the triangle inequality once again \vspace{\jot}}
...
\noalign{\noindent Again via the \cref{as:continuous-Hs-regularity} this leads to}
	\chi &\leq& c \max\{1, \alpha\} h^{\min \{ s, p+1\} - 1} \cdot \Bigg[ c_w c_E c_w'  ( \|\bar{u}\|_{L^2(\Sigma)} + \| y_\Omega \|_{L^2(\Omega)} ) \| \beta \|_{L^\infty(\Sigma)} \| v \|_{L^2(\Sigma)} \\
	&& {}+ \left( \frac{1}{c_S^A} \left( \| \beta \|_{L^\infty(\Sigma)} \| \bar{u} \|_{L^2(\Sigma)} + \| y_\Omega \|_{L^2(\Omega)} \right) + \| y_\Omega \|_{L^2(\Omega)} \right) c_w' \| \beta \|_{L^\infty(\Sigma)} \|v\|_{L^2(\Sigma)} \Bigg]. \\
