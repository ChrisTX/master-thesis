...
\noalign{\noindent Using the previously derived results \cref{thm:L2proj-A-error,thm:L2proj-AR-error,thm:L2proj-Astar-error,thm:L2proj-B-error}, we obtain:}
	\lDG f - f_h \rDG &\leq& \left[ \| f - Q_{\meshT_N} f \|_A^2 + \| f - Q_{\meshT_N} f \|_{A, R}^2 + \| f - Q_{\meshT_N} f \|_B^2 \right]^\frac{1}{2} \\
...
Then the following error estimate holds for some constant $C > 0$
\[
	\| f - f_h \|_{L^2(Q)} \leq C c_g c_2^A \sqrt{1 + c_K} h^{\min \{ s, p+1 \} - 1} \lDG f - f_h \rDG.
\]
\end{theorem}
\begin{proof}
Consider the following auxiliary problem: Find $\chi \in H^1(Q)$ such that
\begin{IEEEeqnarray*}{rCl}
	\iint_Q \left(- \frac{\partial \chi}{\partial t} - \lapl \chi \right) v \dd x \dd t = \iint_Q (f - f_h) v \dd x \dd t \qquad \text{for all $H^1(Q)$},
\end{IEEEeqnarray*}
...
Using our understanding of the adjoint and the assumption that $\chi \in H^s(Q)$ implies that $\chi$ and its derivatives are continuous, we observe that all jump penalty terms in $a^\sip$ and $b$ disappear, simplifying $A(\cdot, \chi)$ to:
\begin{IEEEeqnarray*}{rCl}
	A(f - f_h, \chi) &=& \iint_Q \left( - \frac{\partial \chi}{\partial t} - \lapl \chi \right) (f - f_h) \dd x \dd t \\
...
\begin{IEEEeqnarray*}{rCl}
	...
	&\leq& c_2^A \sqrt{1 + c_K} \lDG f - f_h \rDG \lDGs \chi - Q_{\meshT_N} \chi \rDGs \\
	&\overset{\text{\cref{thm:L2proj-Astar-error}}}{\leq}& c_2^A \sqrt{1 + c_K} \lDG f - f_h \rDG \cdot c \left[ \sum_{\ell=1}^N h_\ell^{2 \min \{ s, p+ 1 \} -2 } |\chi|^2_{H^s(\tau_\ell)} \right]^\frac{1}{2} \\
	&\overset{\text{\cref{as:quasi-uniform}}}{\leq}& C c_g c_2^A \sqrt{1 + c_K} h^{\min \{ s, p+1 \} - 1} \| f - f_h \|_{L^2(Q)} \lDG f - f_h \rDG
\end{IEEEeqnarray*}
...
\[
	\| f - f_h \|_{L^2(Q)} \leq C \max\{ 1, \alpha \} h^{2 \min \{ s, p+1\} - 2} |f|_{H^s(\meshT_N)}.
\]
\end{corollary}
\end{document}