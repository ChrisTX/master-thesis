...
For a function $f \in H^s(\meshT_N)$ with $s > \frac{3}{2}$ we define
\begin{IEEEeqnarray*}{rCl}
...
\end{IEEEeqnarray*}
In the same vein, we define for $b(\cdot, \cdot)$ two norms for functions $f \in H^s(\meshT_N)$, $s \geq 1$:
\begin{IEEEeqnarray*}{rCl}
...
\]
for all $f \in H^s(\meshT_N)$ with $s \geq 1$ and for all $v_h \in \Shp(\meshT_N)$.
\end{lemma}
\begin{proof}
...
\noalign{\noindent Applying \cref{thm:L2-approximation-props} we estimate this as}
	\| f - Q_{\meshT_N} f \|_{L^2(\Sigma_R)}^2 &\leq& c^2 \sum_{\ell=1}^N h_\ell^{2 \min \{ s, p+ 1 \} - 1} |f|^2_{H^s(\tau_\ell)}.
\end{IEEEeqnarray*}
...
\begin{IEEEeqnarray*}{rCl}
	...
	&\overset{\text{\cref{thm:L2proj-Astar-error}}}{\leq}& c_2^A \sqrt{1 + c_K} \lDG f - f_h \rDG \cdot c \left[ \sum_{\ell=1}^N h_\ell^{2 \min \{ 2, p+ 1 \} -2 } |\chi|^2_{H^2(\tau_\ell)} \right]^\frac{1}{2} \\
	&\overset{\text{\cref{as:quasi-uniform}}}{\leq}& C c_g c_2^A \sqrt{1 + c_K} h \| f - f_h \|_{L^2(Q)} \lDG f - f_h \rDG
\end{IEEEeqnarray*}
